...
namespace EateryReviewWebsiteBE.Controllers
{
    ...
    public class ModerationController : ControllerBase
    {
        ...
        public async Task<IActionResult> AssessByAI(int blogId)
        {
            ...
            try
            {
                ...
                if (trimmed.StartsWith("APPROVE", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    await NotificationService.SendAsync(_context, blog.UserId.Value, $"AI đã Approve bài viết '{blog.BlogTitle}' do: {blog.Opinion}", MessageType.Success);
                ...
                else if (trimmed.StartsWith("REJECT", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    await _context.SaveChangesAsync();
                    await NotificationService.SendAsync(_context, blog.UserId.Value, $"AI đã Reject bài viết '{blog.BlogTitle}' do: {blog.Opinion}", MessageType.Failure);
        ...
        public async Task<IActionResult> ReviewBlogAsync(int blogId, [FromBody] ReviewRequestModel reviewRequest)
        {
            ...
            //If the blog is approved, set the paid expiration date
            var action = request == (int)BlogModerationStatus.Approved ? "approved" : "rejected";
            await NotificationService.SendAsync(_context, blog.UserId.Value, $"Moderator đã {action} bài viết '{blog.BlogTitle}' do: {blog.Opinion}", MessageType.Success);

            // Begin the transaction for wallet balance change
            if(blog.PaidOptionId != null && blog.PaidOptionId > 0)
            {
                WalletHelper walletHelper = new WalletHelper(_context);
                await walletHelper.ChangeBalanceAsync((int)blog.UserId, (int)blog.PaidOptionId, (int)blog.BlogId);
            }
