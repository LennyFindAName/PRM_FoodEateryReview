...
namespace EateryReviewWebsiteBE.Controllers
{
    ...
    public class BlogController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateAsync([FromBody] BlogRequestModel blogData)
        {
            ...
            try
            {
                ...
                var result = new Blog
                {
                    ...
                    PaidOptionId = blogData.PaidOptionId == 0 ? null : blogData.PaidOptionId
                };
                ...
                try
                {
                    ...
                    if (aiResponse.IsSuccessStatusCode)
                    {
                        ...
                        if (document.RootElement.TryGetProperty("status", out var statusProperty))
                        {
                            ...
                            if (status == (int)BlogModerationStatus.Approved 
                                && result.PaidOptionId != null 
                                && result.PaidOptionId > 0
                                && user.WalletBalance > blogData.DisplayPaidCost)
                            {
                                WalletHelper walletHelper = new WalletHelper(_context);
                                await walletHelper.ChangeBalanceAsync((int)user.UserId, (int)result.PaidOptionId, (int)result.BlogId);
                            }
                        }
                    }
                }
        ...
        [HttpPut("remove/{id}")]
        public async Task<IActionResult> RemoveBlogAsync(int id)
        {
